...
const AddShows = () => {
  ...
  const fetchNowPlayingMovies = async () => {
    try{
      const { data } = await axios.get('/api/show/movies/now-playing', {headers: { Authorization: `Bearer ${await getToken()}`}})
      if(data.success){
...
<div
  key={movie.id}
  className="relative max-w-40 cursor-pointer hover:opacity-40 hover:-translate-y-1 transition duration-300"
  onClick={() => setSelectedMovie(movie.id)}
>
  <div className="relative pb-10">   {/* <-- FIX ADDED */}
  ...
  <p className="text-gray-400 text-sm">{movie.release_date}</p>
        ...
        <div className='mt-6'>
          <label className='block text-sm font-medium nb-2'>Select Date and Time</label>
              ...
              {Object.entries(dateTimeSelection).map(([date, times])=>(
                <li key={date}>
                  ...
                  <div className='flex flex-wrap gap-2 mt-1 text-sm'>{times.map((time) =>(
                    <div key={time} className='border border-primary px-2 py-1 flex items-center rouneded '>
                      <span>{time}</span>
                      <DeleteIcon onClick={()=> handleRemoveTime(date, time)} width={15} className='ml-2 text-red-500 hiver:text-red-700 cursor-pointer' />
                    </div>
                  ))}</div>
                </li>
              ))}
